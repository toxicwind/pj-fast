...
The `pj/` run artifacts (when added — pending while the recon host link is
down: `out21`–`out49` outputs, `final-winner-storage.json` and
`out38/storage.json` Playwright storage snapshots of the verified $27.11
BOGO cart) can rehydrate a Playwright browser context to the last verified
cart state. Nothing here can re-submit an order on its own —
checkout/payment was never completed.
